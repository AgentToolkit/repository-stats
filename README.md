...
Analytics dashboard for the CUGA project, displaying GitHub traffic, stars, and PyPI download statistics.
...
3. Configure the token:
   - **Name**: CUGA Stats Traffic Data
   - **Expiration**: Set according to your preference (recommended: 90 days or 1 year)
   - **Repository access**: Select "Only select repositories" and choose `cuga-project/cuga-agent`
   - **Permissions**:
...
**Note**: You must have admin or maintain role on `cuga-project/cuga-agent` for the token to access traffic data.
...
1. Go to your repository settings: `https://github.com/cuga-project/cuga-stats/settings/secrets/actions`
...
python download_traffic.py --repo cuga-project/cuga-agent --output-dir traffic_data_latest

# Or specify token directly (not recommended for scripts)
python download_traffic.py --repo cuga-project/cuga-agent --output-dir traffic_data_latest --token YOUR_TOKEN
```
...
1. Go to [Actions > Fetch Traffic Data](https://github.com/cuga-project/cuga-stats/actions/workflows/fetch-traffic-data.yml)
...
1. **Check your repository role**: You need admin or maintain role on `cuga-project/cuga-agent`
...
- Verify the repository name is correct: `cuga-project/cuga-agent`
...
Live site: https://cuga-project.github.io/cuga-stats/
